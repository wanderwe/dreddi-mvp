...
| **Invite ignored / expired** | Yes | auto-expire ignored invites | In cron: active + `invite_status=awaiting_acceptance` + not accepted + `ignored_at is null` + `created_at <= now - INVITE_TTL_HOURS` (default 7 days; override via `inviteIgnoreMinutes`). Then updates promise to `status=declined`, `invite_status=expired`, sets `ignored_at`; creates `invite_ignored` notification for creator. | Creator only. | Yes. | No by design (`invite_ignored` not email-eligible). | One-time due to terminal status + state. | **Working** (subject to cron actually running). |
