-- Security hardening: enable RLS for deal_invites and tighten public profile stats exposure.

ALTER TABLE public.deal_invites ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS deal_invites_participant_select ON public.deal_invites;
CREATE POLICY deal_invites_participant_select
  ...
  USING (
    ...
    OR EXISTS (
      ...
      WHERE p.id = deal_invites.deal_id
        AND auth.uid() IN (p.creator_id, p.counterparty_id, p.promisor_id, p.promisee_id)
    )
  );

DROP POLICY IF EXISTS deal_invites_creator_insert ON public.deal_invites;
CREATE POLICY deal_invites_creator_insert
  ON public.deal_invites
  FOR INSERT
  WITH CHECK (
    auth.uid() = inviter_id
...
DROP POLICY IF EXISTS deal_invites_participant_update ON public.deal_invites;
CREATE POLICY deal_invites_participant_update
  ON public.deal_invites
  FOR UPDATE
  USING (
    auth.uid() = inviter_id
...
DROP POLICY IF EXISTS deal_invites_creator_delete ON public.deal_invites;
CREATE POLICY deal_invites_creator_delete
  ON public.deal_invites
  FOR DELETE
  USING (
    auth.uid() = inviter_id
...
ALTER VIEW public.reputation_scores_by_user
