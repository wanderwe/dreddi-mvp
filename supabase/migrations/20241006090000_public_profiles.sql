-- Public profile views for anonymous access
...
CREATE OR REPLACE VIEW public_profile_deals AS
SELECT
  promises.id,
  promises.title,
  promises.status,
...
FROM promises
WHERE promises.status IN ('confirmed', 'disputed');

GRANT SELECT ON public_profile_stats TO anon, authenticated;
GRANT SELECT ON public_profile_deals TO anon, authenticated;
