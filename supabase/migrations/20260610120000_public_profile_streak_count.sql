-- Add current_streak_count to public_profile_stats: the number of most-recent
-- consecutive confirmed deals (as executor), counting across ALL finalized deals
-- (public and private alike) — only the resulting number is exposed, no
...
streak_ranked AS (
  SELECT
    executor_id,
    status,
    ROW_NUMBER() OVER (PARTITION BY executor_id ORDER BY finalized_at DESC) AS rn
  FROM public_deals
),
streak_breaks AS (
  SELECT executor_id, MIN(rn) AS first_break_rn
  FROM streak_ranked
  WHERE status = 'disputed'
  GROUP BY executor_id
),
streak_totals AS (
  SELECT executor_id, COUNT(*) AS total_rn
  FROM streak_ranked
  GROUP BY executor_id
),
streak_metrics AS (
  SELECT
    streak_totals.executor_id,
    COALESCE(streak_breaks.first_break_rn - 1, streak_totals.total_rn) AS current_streak_count
  FROM streak_totals
  LEFT JOIN streak_breaks ON streak_breaks.executor_id = streak_totals.executor_id
)
SELECT
  ...
  COALESCE(streak_metrics.current_streak_count, 0::bigint) AS current_streak_count
...
LEFT JOIN streak_metrics ON streak_metrics.executor_id = profiles.id
