...
export async function GET(_req: Request, ctx: { params: Promise<{ token: string }> }) {
  try {
    ...
    const { data: p, error } = await admin
      .from("promises")
      .select(
        "id,title,details,due_at,status,created_at,creator_id,counterparty_id,invite_token,counterparty_contact,public_requested"
      )
    ...
    return NextResponse.json(
      {
        ...
        public_requested: p.public_requested ?? false,
      },
      { status: 200 }
    );
  } catch (e) {
