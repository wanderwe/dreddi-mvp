...
export async function loadPromiseForUser(id: string, userId: string) {
  ...
  const { data: promise, error } = await admin
    .from("promises")
    .select(
      "id,title,details,status,promise_mode,due_at,creator_id,counterparty_id,promisor_id,promisee_id,completed_at,confirmed_at,disputed_at,disputed_code,dispute_reason,condition_text,condition_met_at,condition_met_by,invite_status,invited_at,accepted_at,declined_at,ignored_at"
    )
    .eq("id", id)
