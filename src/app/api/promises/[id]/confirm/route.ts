...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: updatedPromise, error } = await admin
      ...
      .eq("id", id)
      .select(
        "id,title,status,due_at,completed_at,creator_id,counterparty_id,promisor_id,promisee_id,confirmed_at,disputed_at,disputed_code,dispute_reason,invite_status,invited_at,accepted_at,counterparty_accepted_at,declined_at,ignored_at,is_high_stake,high_stake_enabled_at"
      )
